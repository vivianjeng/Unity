...
int main()
{
   ...
   if(query == "accept"){
     int accept_u, accept_i, accept_t;
     cin>>accept_u>>accept_i>>accept_t;
   ...
   while (low <= high)      //binary search to find Result
   {
     ...
     int items_u1, items_u2;
     cin>>items_u1>>items_u2;
     ...
     int users_i1, users_i2, users_t1, users_t2;
     cin>>users_i1>>users_i2>>users_t1>>users_t2;
...
/* failed
     ...
     int ft_i;
     std::vector<int> ft_users;
     cin>>ft_i;
     int user;
  /*  failed
   while(cin >> user){
        ft_users.push_back(user);
        if(getchar() == '\n')
          break;
      }
           ...
           for(int i=0;i<ft_users.size();i++)
              if(N[mid1].UserId == ft_users[i]){
                ...
                for (int k = 0; k < down1+up1-1 ; k++) {
                      tmp.push_back(N[mid1].Unix_timestamp);
                }
